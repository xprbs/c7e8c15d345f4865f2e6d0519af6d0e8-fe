...
const MSIFormRegistration = () => {
    ...
    const fetchType = async (token, currentData = null) => {
        try {
            ...
            if (currentData != null) {
                const filteredData = data.filter(item => item.ParentProductCategoryCode === currentData.ditem_type);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (token) {
                ...
                const type = await fetchType(token, itemResponse.data);
                setToken(token);
                setUomOptions(uom);
                setTypeOptions(type);
            }
    ...
    const handleJustSubmit = async (event, status) => {
        ...
        try {
            ...
            router.push('/item/msi-registration');
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    const handleSaveAndSubmit = async (event) => {
        ...
        try {
            ...
            const response = await backendApi.post(`/web/items/updateregitrasi/${id}`, updatedFormData);
            const newItem = response.data;
                    ...
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} md={4}>
                            <TextField
                                label='Dimension'
                                name='dimension'
                                value={formData.dimension === 'CoSiMaSt' ? 'Config | Size | Material/Color | Style' : formData.dimension}
                                onChange={handleInputChange}
                        ...
                        <Grid item xs={12} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Button variant='outlined' onClick={() => router.push('/item/msi-registration')}>
                                Back
                            </Button>
...
MSIFormRegistration.acl = {
    action: 'manage',
    subject: 'msi-registration-form'
};

export default MSIFormRegistration;
