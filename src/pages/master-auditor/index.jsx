...
const schema = yup.object().shape({
  auditor_name: yup.string().required('Auditor Name is a required field')
  //   auditor_category: yup.string().required('Auditor Category is a required field')
})
...
const AuditCategoryPage = () => {
  ...
  //   const [auditorcategory, setAuditorCategory] = useState(null)
  //   const [auditorcategoryId, setAuditorCategoryId] = useState(null)
  const [auditorname, setAuditorName] = useState(null)
  const [auditornameId, setAuditorNameId] = useState(null)
  ...
  const updateHandler = e => {
    ...
    const myPromise = new Promise((resolve, reject) => {
      backendApi
        .post('/web/master/auditor/update', dataForm)
        .then(res => {
          resolve('success')
  ...
  const createHandler = e => {
    ...
    const dataForm = JSON.stringify({
      auditor_name: auditornameId?.id
      //   auditor_category: auditorcategoryId?.id
    })

    const createPromise = new Promise((resolve, reject) => {
      backendApi
        .post('/web/master/auditor/store', dataForm)
        .then(res => {
          resolve('success')
          updateData('reload', !data.reload)
          setCreateData('')
          //   setAuditorCategoryId(null)
          setAuditorNameId(null)
        })
        .catch(error => {
          reject(error)
        })
  ...
  useEffect(() => {
    const initData = async () => {
      ...
      await backendApi
        .post('/web/master/auditor/list', dataForm)
        .then(response => {
  ...
  async function getInitName() {
    new Promise((resolve, reject) => {
      backendApi
        .post('/web/master/auditor/userlist')
        .then(res => {
  ...
  //     new Promise((resolve, reject) => {
  //       backendApi
  //         .post('/web/master/auditor/category')
  //         .then(res => {
  //           setAuditorCategory(res.data.data)
  //           resolve('success')
  //         })
  //         .catch(error => {
  //           console.log(error)
  ...
  return (
    <>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <PageHeader title={<Typography variant='h5'>Master Auditor</Typography>} subtitle={null} />
        </Grid>
        <Grid item xs={12}>
          <Card sx={{ mb: 6 }}>
                    ...
                    <Autocomplete
                      ...
                      renderInput={params => (
                        <TextField
                          ...
                          error={Boolean(errors.auditor_name)}
                        />
                      )}
                      onChange={(event, newValue) => {
                        setAuditorNameId(newValue)
                      }}
                      isOptionEqualToValue={(option, value) => option.id === value.id}
                      value={auditornameId}
                    />
                    {errors.auditor_name && (
                      <FormHelperText sx={{ color: 'error.main' }}>{errors.auditor_name.message}</FormHelperText>
                    )}
                  </FormControl>
                </Grid>
                {/* <Grid item xs={12} md={4}>
                  <FormControl fullWidth>
                    <Autocomplete
                      size='small'
                      options={auditorcategory}
                      fullWidth
                      renderInput={params => (
                        <TextField
                          {...params}
                          {...register('auditor_category')}
                          label='Auditor Category'
                          placeholder='Auditor Category'
                          InputLabelProps={{ shrink: true }}
                          error={Boolean(errors.auditor_category)}
                        />
                      )}
                      onChange={(event, newValue) => {
                      ...
                      isOptionEqualToValue={(option, value) => option.id === value.id}
                      value={auditorcategoryId}
                    />
                    {errors.auditor_category && (
                      <FormHelperText sx={{ color: 'error.main' }}>{errors.auditor_category.message}</FormHelperText>
                    )}
                  </FormControl>
                </Grid> */}
                <Grid item={true}>
