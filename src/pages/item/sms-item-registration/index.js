...
import { fetchUom, getDynamicApiToken } from 'src/helpers/dynamicApi'

const SMSItemRegistration = () => {
    ...
    },
    {
      ...
      renderCell: params => (
        <Box display='flex' justifyContent='space-between'>
          ...
          <Link href={`/item/sms-item-registration/form/${params.row.id_item}`} passHref>
  ...
  return (
    <Grid container spacing={6}>
      <PageHeader
        title={<Typography variant='h5'>SMS Registration</Typography>}
        subtitle={<Typography variant='body2'>List of all SMS registrations</Typography>}
      />
      <Grid item xs={12}>
        <Card>
          <Box
          ...
          >
            <Button
              ...
              component={Link}
              variant='contained'
              href='/item/sms-item-registration/form'
              size='small'
            >
              Create Menu
            </Button>
      ...
      >
        ...
        <DialogContent>
          ...
          <TableContainer>
            <Table>
              <TableBody>
                {Object.entries(dataApproval).map(
                  ([key, value]) =>
                    columnMapping[key] ? (
                      <TableRow key={key}>
                        <TableCell style={{ border: '1px solid black' }}>{columnMapping[key]}</TableCell>
                        <TableCell style={{ border: '1px solid black' }}>{value}</TableCell>
                      </TableRow>
...
SMSItemRegistration.acl = {
  action: 'manage',
  subject: 'sms-item-registration'
}

export default SMSItemRegistration
