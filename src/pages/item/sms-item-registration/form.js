...
import { fetchBasicData, fetchProductCategories, fetchUom, getDynamicApiToken } from 'src/helpers/dynamicApi'
...
const SMSFormRegistration = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        console.log(basicData.stock_group)
  ...
  const handleStockGroupChange = async event => {
    const { value } = event.target
    setFormData(prevState => ({ ...prevState, stock_group: value, stock_group: value }))

    try {
      const data = stockGroupOptions
      const filteredData = data.filter(item => item.stock_group === value)
      setSubStockGroupOptions(filteredData)
    } catch (error) {
      console.error(error)
    }
  }
  ...
  const handleSaveAndSubmit = async event => {
    ...
    try {
      ...
      console.log('momok', updatedFormData)

      const response = await backendApi.post('/web/sms', updatedFormData)
      if (response.status === 201) {
        router.push('/item/sms-item-registration')
      } else {
        alert('error!')
      }
    } catch (error) {
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <Box style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                ...
                <Button variant='contained' color='primary' onClick={handleSaveAndSubmit} style={{ marginLeft: 10 }}>
                  Submit And Approval
                </Button>
              </Box>
            </Grid>
